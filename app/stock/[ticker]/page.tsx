...
interface StockDetail { ticker: string; name: string; nameEn: string; sector: string; price: number; change: number; changePct: number; per: number; perFwd: number; pbr: number; dividend: number; dividendYield: number; marketCap: number; eps: number; roe: number; high52: number; low52: number; waterLevel: number; history: { date: string; value: number }[]; source: string }

const CustomTooltip = ({ active, payload, label }: any) => {
  if (!active || !payload?.length) return null
  return (<div style={{ background: 'rgba(13,15,20,0.95)', border: '0.5px solid rgba(196,156,72,0.3)', padding: '8px 12px', borderRadius: 8, fontSize: 12 }}><div style={{ color: '#6B7280', marginBottom: 4 }}>{label}</div><div style={{ color: '#C49C48', fontWeight: 500 }}>¥{payload[0].value?.toLocaleString()}</div></div>)
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      ...
      <div style={{ position: 'relative', width: 48, height: 120, background: 'rgba(255,255,255,0.06)', borderRadius: 24, overflow: 'hidden', border: '0.5px solid rgba(255,255,255,0.1)' }}>
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: `${pct}%`, background: `linear-gradient(to top, ${color}99, ${color}ht 1s ease', borderRadius: '0 0 24px 24px' }} />
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ fontSize: 13, fontWeight: 600, color: '#E8E4D9' }}>{pct}%</span></div>
      </div>
...
function MetricCard({ label, value, sub, highlight }: { label: string; value: string; sub?: string; highlight?: boolean }) {
  return (
    <div style={{ background: highlight ? 'rgba(196,156,72,0.06)' : 'rgba(255,255,255,0.03)', border: `0.5px solid ${highlight ? 'rgba(196,156,72,0.25)' : 'rgba(255,255,255,0.07)'}`, borderRadius: 10, padding: '14px 16px' }}>
    ...
    <div style={{ minHeight: '100vh', background: '#0D0F14', padding: '24px', fontFamily: "'DM Sans', system-ui, sans-serif", color: '#E8E4D9' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 28 }}>
          <button onClick={() => router.push('/')} style={{ background: 'rgba(255,255,255,0.04)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '8px 14px', color: '#B8B4A8', cursor: 'pointer', fontSize: 13 }}>← 戻る</button>
          {!loading && data && (
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, flexWrap: 'wrap' }}>
              <h1 style={{ fontFamily: 'DM Serif Display, serif', fontSize: 26, margin: 0, color: '#E8E4D9' }}>{data.name}</h1>
        ...
        {!loading && data && (
          <>
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 16, marginBottom: 24, flexWrap: 'wrap' }}>
              <div style={{ fontFamily: 'DM Serif Display, serif', fontSize: 42, color: '#C49C48', lineHeight: 1 }}>¥{data.price.toLocaleString()}</div>
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '0.5px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems:'center', marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'flex', gap: 4 }}>
                    {RANGES.map(r => (<button key={r} onClick={() => setRange(r)} style={{ fontSize: 11, padding: '4px 10px', borderRadius: 6, cursor: 'pointer', border: 'none', background: range === r ? 'rgba(196,156,72,0.15)' : 'transparent', color: range === r ? '#C49C48' : '#6B7280' }}>{RANGE_LABELS[r]}</button>))}
                  </div>
                </div>
                <ResponsiveContainer width="100%" height={240}>
                  <AreaChart data={data.history} margin={{ top: 5, right: 5, bottom: 0, left: 0 }}>
                    <defs><linearGradient id="goldGrad2" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#C49C48" stopOpacity={0.25} /><stop offset="95%" stopColor="#C49C48" stopOpacity={0} /></linearGradient></defs>
                    <CartesianGrid strokeDasharray="3 3" strok255,255,0.04)" vertical={false} />
                ...
                <div style={{ marginTop: 16, padding: '12px 16px', background: 'rgba(255,255,255,0.02)', borderRadius: 8 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    <span style={{ fontSize: 11, color: '#4ADE80' }}>安値 ¥{data.low52.toLocaleString()}</span>
                    <span style={{ fontSize: 11, color: '#6B7280' }}>現在 ¥{data.price.toLocaleString()}</span>
                    <span style={{ fontSize: 11, color: '#F87171' }}>高値 ¥{data.high52.toLocaleString()}</span>
                  </div>
                  <div style={{ position: 'relative', height: 6, background: 'rgba(255,255,255,0.06)', borderRadius: 3 }}>
                    ...
                    <div style={{ position: 'absolute', top: -3,waterLevel}%`, transform: 'translateX(-50%)', width: 12, height: 12, borderRadius: '50%', background: '#E8E4D9', border: '2px solid #0D0F14' }} />
                  </div>
                </div>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 12 }}>
              ...
              <MetricCard labelcaleString()}`} sub={`ROE ${data.roe}%`} />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '0.5px solid rgba(255,255,255,0.07)', borderRadius: 10, padding: '14px 16px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <div><div style={{ fontSize: 11, color: '#4ADE80', marginBottom: 2 }}>安値</div><div style={{ fontSize: 18, fontFamily: 'DM Serif Display, serif', color: '#E8E4D9' }}>¥{data.low52.toLocaleString()}</div></div>
                  ze: 11, color: '#F87171', marginBottom: 2 }}>高値</div><div style={{ fontSize: 18, fontFamily: 'DM Serif Display, serif', color: '#E8E4D9' }}>¥{data.high52.toLocaleString()}</div></div>
                </div>
              </div>
            </div>
          </>
