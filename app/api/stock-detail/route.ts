...
const STOCK_META: Record<string, { name: string; nameEn: string; sector: string; base: number; high52: number; low52: number; per: number; perFwd: number; pbr: number; dividend: number; dividendYield: number; marketCap: number; eps: number; roe: number; }> = {
  ...
  '8306.T': { name: '三菱UFJフィナンシャル', nameEn: 'MUFG', sector: '銀行業', base: 1580, high52: 1820, low52: 1120, per: 11.2, perFwd: 10.4, pbr: 0.9, divide
  ...
  '9984.T': { name: 'ソフトバンクグループ', nameEn: 'SoftBank Group', sector: '情報・通信業', base: 9200, high52: 11200, low52: 7400, per: 42.1, perFwd: 32.4, pbr: 1.4, dividend: 44, dividendYield: 0.48, marketCap: 15800, eps: 218, roe: 3yence', sector: '電気機器', base: 65000, high52: 75000, low52: 52000, per: 45.2, perFwd: 40.1, pbr: 7.2, dividend: 320, dividendYield: 0.49, marketCap: 158000, eps: 1438, roe: 16.8 },
}
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (result) {
        ...
        history = timestamps.map((t: number, i: number) => ({ date: new Date(t * 1000).toLocaleDateString('ja-JP', { month: 'short', day: 'numeric' }), value: closes[i] ? parseFloat(closes[i].toFixed(0)) : null })).filter((p: any): p is { date: string; value: number } => p.value !== null)
        const m = result.meta
